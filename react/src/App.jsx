...
function PublicLandingPage({ onOpenAuth }) {
  ...
  useEffect(() => {
    ...
    const heroMembershipButton = document.querySelector('.hero-actions .primary-button');
    const scrollToTiers = () => mount.scrollIntoView({ behavior: 'smooth' });
    heroMembershipButton?.addEventListener('click', scrollToTiers);
    return () => {
      heroMembershipButton?.removeEventListener('click', scrollToTiers);
      root.unmount();
      mount.remove();
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const actions = quickLinksBar?.parentElement;
    if (!quickLinksBar || !actions) return undefined;

    quickLinksBar.setAttribute('role', 'button');
    quickLinksBar.setAttribute('tabindex', '0');
    quickLinksBar.setAttribute('aria-label', 'فتح قائمة الروابط السريعة');
    const toggleMenu = (event) => {
      if (event.target.closest('a')) return;
      actions.classList.toggle('menu-open');
      quickLinksBar.setAttribute('aria-expanded', actions.classList.contains('menu-open'));
    };
